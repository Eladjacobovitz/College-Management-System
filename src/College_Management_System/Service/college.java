 ...
 * (See the CourseService, StudentService, and CourseTakenSService classes for more details).
 */
public class CollegeService {
		private static String nl =  System.getProperty("line.separator");
		private String name; // the name of this college
		private LinkedList<CourseService> courses;
		private LinkedList<StudentService> students;
		...
		public CollegeService(String name) {
			this.name = name;
			this.courses = new LinkedList<CourseService>();
			this.students = new LinkedList<StudentService>();
		}
		...
		public void addCourse(int cid, String title) {
			...
			CourseService newcourse = new CourseService(cid,title);
		...
		public void topPerfomerReport(int cid) {
			...
			CourseService c = getCourse(cid);
			ListIterator <StudentService> itr = students.iterator();
			...
			while(itr.hasNext()) {
				StudentService student = itr.next();
			...
			ListIterator <StudentService> itr2 = students.iterator();
			...
			while(itr2.hasNext()) {
				StudentService student = itr2.next();
		...
		public boolean removeCourse(int cid) {
			CourseService coursetoremove = getCourse(cid);
			if(coursetoremove == null) return false;
			else{
				courses.remove(coursetoremove);
				ListIterator<StudentService> itr = students.iterator();
				while(itr.hasNext()) {
					StudentService current = itr.next();
		...
		public void addStudent(int sid, String name) {
			...
			StudentService newstudent = new StudentService(sid,name);
		...
		public boolean removeStudent(int sid) {
			StudentService studenttoremove = getStudent(sid);
		...
		public void addCourseTaken(int sid, int cid, int grade) {
			...
			StudentService student = getStudent(sid);
			CourseService course = getCourse(cid);
		...
		private LinkedList<StudentService> studentsWhoTook(CourseService c) {
			// First we will create a list of all the students who took the course
			LinkedList<StudentService> studentsWhoTookcourse = new LinkedList<StudentService>();
			ListIterator <StudentService> itr = students.iterator();
			while (itr.hasNext()) {
				StudentService student = itr.next();
		 ...
		 * @param cid student id
		 */
		private CourseService getCourse(int cid) {
			ListIterator <CourseService> itr = courses.iterator();
			while(itr.hasNext()) {
				CourseService course = itr.next();
		 ...
		 * @param cid student id
		 */
		private StudentService getStudent(int sid) {
			ListIterator <StudentService> itr = students.iterator();
			while(itr.hasNext()) {
				StudentService student = itr.next();
